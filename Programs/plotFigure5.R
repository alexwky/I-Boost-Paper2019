...
for (method in c("IBcv","IBperm")) {
	...
	all.NRI <- lapply(1:3,function(j)matrix(NA,16,30))
	GE.index <- intersect(grep("Clinical",NAMES.95),grep("GeneExp",NAMES.95))
	for (type in c(1,2,3)) {
		...
		for (i in GE.index) {
			...
			index <- unlist(strsplit(name,split="\\."))[1]
			moduleindex <- gsub("0","1",index)
			modulename <- setdiff(unlist(strsplit(name,split="\\."))[-1],"GeneExp")
			modulename <- paste(c("Module",modulename),collapse=".")
			name2 <- paste(moduleindex,modulename,sep=".")
			i2 <- which(name2 == NAMES.95)
			features2 <- FEATURES.95[[i2]]
			features.t2 <- setdiff(features2, rm.features)
		
			for (s in 1:30) {
				GE.test.modules <- t(x$xd[features.t, filter.t & split[,s] == 1])
				beta <- try(read.table(paste(indir, "/Model_split",s,"-",type.name,"-",name,".csv",sep=""), header=FALSE, sep=",", row.names=1))
				if (class(beta) == "try-error") next
				if (nrow(beta) > 0) preds <- as.matrix(GE.test.modules[,match(rownames(beta),x$fnames[features.t])]) %*% beta[,1] else preds <- rep(0, nrow(GE.test.modules))
			
				module.test.modules <- t(x$xd[features.t2, filter.t & split[,s] == 1])
				beta2 <- try(read.table(paste(indir, "/Model_split",s,"-",type.name,"-",name2,".csv",sep=""), header=FALSE, sep=",", row.names=1))
				if (class(beta2) == "try-error") next
				if (nrow(beta2) > 0) preds2 <- as.matrix(module.test.modules[,match(rownames(beta2),x$fnames[features.t2])]) %*% beta2[,1] else preds2 <- rep(0, nrow(module.test.modules))
	
				test.outcome <- Surv(x$outcome[filter.t&split[,s]==1,2],x$outcome[filter.t&split[,s]==1,1])
				IDINRI <- IDI.INF(test.outcome,preds,preds2,thres,npert=1)
				all.NRI[[type]][j,s] <- IDINRI$m2[1]
			}
			j <- j + 1
		}
	}
	...
	rownames(result.NRI) <- NAMES.95[GE.index]
	all.methods.NRI <- c(all.methods.NRI, list(result.NRI))
}
...
pdf("../Plots/Figure5.pdf", width=7, height=14) 
par(mfrow=c(2,1))
