...
write.table(file="../SimulationData/allPredictorValues.csv",all.data,sep=",",row.names=FALSE,col.names=TRUE,quote=FALSE)
EOF
...
for (a in alpha) {
	cvm <- rep(0, 100)
	for (j in 1:n.repeat.cv) {
		cv.fit.en <- cv_glmnet(all.modules, all.outcome, family="cox", alpha=a, foldid=foldids[[j]], error="cox")
