	...
	if ("$method" == "lasso" | "$method" == "en") {
		n.repeat.cv <- 5
	        ...
	        for (a in alpha) {
	                cvm <- rep(0, 100)
	                for (j in 1:n.repeat.cv) {
	                        cv.fit.en <- cv_glmnet(train.modules, train.outcome, family="cox", alpha=a, foldid=foldids[[j]], error="cox")
		...
		ib <- IBoost(train.modules,train.outcome,sub.data.types,method=method,iter.max=2000,v=v,alpha.series=alpha.series,seed=123987*i)
		beta <- ib\$beta
		it <- ib\$iter.no
		tuning <- c(it,NA)
	}
